...
export const UserProvider = ({ children }) => {
  ...
  const addToCart = async (product) => {
    ...
    try {
      await fetch(`http://localhost:3000/cart?userId=${user.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateUser = async (updatedUserData) => {
    try {
      ...
      toast.success('اطلاعات با موفقیت ذخیره شد!', {
        position: "top-center", // یا "bottom-center" بسته به جایی که می‌خوای نمایش داده بشه
        autoClose: 1500, // مدت زمان بسته شدن خودکار (اختیاری)
      });
    } catch (error) {
  ...
  const removeFromCart = async (productToRemove) => {
    ...
    toast.success('اطلاعات با موفقیت ذخیره شد!', {
  position: "top-center", // یا "bottom-center" بسته به جایی که می‌خوای نمایش داده بشه
  autoClose: 2000, // مدت زمان بسته شدن خودکار (اختیاری)
});
    try {
      await fetch(`http://localhost:3000/cart?userId=${user.id}`, {
        method: 'PUT',
  ...
  const removeFromLikes = async (productToRemove) => {
    ...
    try {
      await fetch(`http://localhost:3000/likes?userId=${user.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
